...
impl MutationRoot {
    ...
    fn lobby_msg(context: &Context, input: ScNewLobbyMessage) -> FieldResult<String> {
        let mut conn = DB_POOL.get().unwrap();
        let user = get_user_basic(&mut conn, context.user_id)?;
        notify_ids(
    ...
    fn voice_msg(context: &Context, input: ScVoiceMsgReq) -> FieldResult<String> {
        ...
        if let Some(room_id) = get_playing(&mut conn, user_id).map(|room| room.id) {
    ...
    fn create_comment(context: &Context, input: ScNewComment) -> FieldResult<ScComment> {
        let mut conn = DB_POOL.get().unwrap();
        create_comment(&mut conn, context.user_id, &input)
    }
    fn create_message(context: &Context, input: ScNewMessage) -> FieldResult<ScMessage> {
        let mut conn = DB_POOL.get().unwrap();
        let message = create_message(&mut conn, context.user_id, &input)?;
        notify(
            message.target_id,
            ScNotifyMessageBuilder::default()
    ...
    fn read_message(context: &Context, input: ScReadMessage) -> FieldResult<ScFriend> {
        let mut conn = DB_POOL.get().unwrap();
        read_message(&mut conn, context.user_id, input.target_id)
    }
    fn favorite_game(context: &Context, input: ScNewFavorite) -> FieldResult<String> {
        let mut conn = DB_POOL.get().unwrap();
        if input.favorite {
            create_favorite(&mut conn, context.user_id, input.game_id).ok();
            notify(
                context.user_id,
                ScNotifyMessageBuilder::default()
            ...
            delete_favorite(&mut conn, context.user_id, input.game_id);
            notify(
                context.user_id,
                ScNotifyMessageBuilder::default()
    ...
    fn apply_friend(context: &Context, input: ScNewFriend) -> FieldResult<String> {
        let mut conn = DB_POOL.get().unwrap();
        if let Ok(target_user) = get_user_by_username(&mut conn, &input.username) {
            if context.user_id != target_user.id {
                match apply_friend(&mut conn, context.user_id, target_user.id) {
                    Ok(friend) => {
                        notify(
                            target_user.id,
                            ScNotifyMessageBuilder::default()
    ...
    fn accept_friend(context: &Context, input: ScUpdateFriend) -> FieldResult<String> {
        let mut conn = DB_POOL.get().unwrap();
        if input.accept {
            if let Ok(friend) = accept_friend(&mut conn, context.user_id, input.target_id) {
                notify(
                    input.target_id,
                    ScNotifyMessageBuilder::default()
            ...
            delete_friend(&mut conn, context.user_id, input.target_id);
            notify(
                input.target_id,
                ScNotifyMessageBuilder::default()
    ...
    fn create_invite(context: &Context, input: ScNewInvite) -> FieldResult<String> {
        let mut conn = DB_POOL.get().unwrap();
        let room_id = get_playing(&mut conn, input.target_id).map(|room| room.id);
        if Some(input.room_id) != room_id {
            match create_invite(&mut conn, context.user_id, &input) {
    ...
    fn accept_invite(context: &Context, input: ScUpdateInvite) -> FieldResult<String> {
        let mut conn = DB_POOL.get().unwrap();
        let invite = get_invite(&mut conn, context.user_id, input.invite_id)?;

        if input.accept {
            let (room_id, room_host) = get_playing(&mut conn, context.user_id)
            ...
            if room_id != invite.room.id {
                ...
                enter_room(&mut conn, context.user_id, invite.room.id);
                notify_ids(
                    get_friend_ids(&mut conn, context.user_id),
                    ScNotifyMessageBuilder::default()
                        .update_user(get_user_basic(&mut conn, context.user_id)?)
                        .build()
                        .unwrap(),
                );
            }
        } else {
            delete_invite_by_id(&mut conn, context.user_id, input.invite_id);
        }
        Ok("Ok".into())
    }
    fn create_room(context: &Context, input: ScNewRoom) -> FieldResult<ScRoomBasic> {
        let mut conn = DB_POOL.get().unwrap();
        let room = create_room(&mut conn, context.user_id, &input)?;
        notify_ids(
            get_friend_ids(&mut conn, context.user_id),
            ScNotifyMessageBuilder::default()
                .update_user(get_user_basic(&mut conn, context.user_id)?)
    ...
    fn update_room(context: &Context, input: ScUpdateRoom) -> FieldResult<ScRoomBasic> {
        let mut conn = DB_POOL.get().unwrap();
        let room = update_room(&mut conn, context.user_id, &input)?;
        notify_ids(
            get_friend_ids(&mut conn, context.user_id),
            ScNotifyMessageBuilder::default()
                .update_user(get_user_basic(&mut conn, context.user_id)?)
        ...
        notify_ids(
            get_room_user_ids(&mut conn, input.id),
            ScNotifyMessageBuilder::default()
                .update_room(get_room(&mut conn, input.id)?)
                .build()
                .unwrap(),
        );
    ...
    fn update_room_screenshot(
        ...
        let mut conn = DB_POOL.get().unwrap();
        Ok(update_room_screenshot(&mut conn, context.user_id, &input)?)
    }
    fn enter_pub_room(context: &Context, input: ScUpdatePlaying) -> FieldResult<ScRoomBasic> {
        let mut conn = DB_POOL.get().unwrap();
        let room = get_room(&mut conn, input.room_id)?;
        if room.private {
            return Err("private room".into());
        }
        enter_room(&mut conn, context.user_id, input.room_id);
        notify_ids(
            get_friend_ids(&mut conn, context.user_id),
            ScNotifyMessageBuilder::default()
                .update_user(get_user_basic(&mut conn, context.user_id)?)
...
pub fn leave_room_and_notify(user_id: i32) -> FieldResult<String> {
    let mut conn = DB_POOL.get().unwrap();
    let room = get_playing(&mut conn, user_id).ok_or(FieldError::new(
        format!("{} not playing", user_id),
        Error::username_not_playing(),
    ))?;
    let invites = get_invites_with(&mut conn, user_id);
    leave_room(&mut conn, user_id, room.id);
    ...
    notify_ids(
        ...
        ScNotifyMessageBuilder::default()
            .update_user(get_user_basic(&mut conn, user_id)?)
            .build()
            .unwrap(),
    );
    Ok("Ok".into())
...
impl Subscription {
    async fn event(context: &Context) -> FriendSysStream {
        ...
        let mut conn = DB_POOL.get().unwrap();
        if let Ok(user) = get_user_basic(&mut conn, context.user_id) {
            notify_ids(
                get_friend_ids(&mut conn, context.user_id),
                ScNotifyMessageBuilder::default()
                    .update_user(user)
...
impl GuestMutationRoot {
    fn register(context: &GuestContext, input: ScRegisterReq) -> FieldResult<ScLoginResp> {
        let mut conn = DB_POOL.get().unwrap();
        register(&mut conn, input, &context.secret)
    }
    ...
    fn login(context: &GuestContext, input: ScLoginReq) -> FieldResult<ScLoginResp> {
        ...
        let resp = login(&mut conn, input, &context.secret)?;
        if !disable_sso {
            notify(
                resp.user.id,
                ScNotifyMessageBuilder::default()
